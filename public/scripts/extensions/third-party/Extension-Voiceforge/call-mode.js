    ...
    resolveCallParticleAmbientAudioUrl(style)
        .then((url) => {
            ...
            try {
                ...
                player.preload = 'auto';
                player.loop = true;
                player.volume = toParticleAmbientVolume(getHypnoAmbientParticleVolume());
...
async function buildWhisperRequestBody(phrase, charName) {
    ...
    const backend = (selectedVoice.tts_backend === 'omnivoice_onnx' || selectedVoice.tts_backend === 'omnivoice_onnx_gpu')
        ? 'omnivoice'
        : selectedVoice.tts_backend;
    if (!backend) {
...
function toParticleAmbientVolume(raw) {
    const clamped = Math.max(0, Math.min(1, Number(raw) || 0));
    return clamped * 0.35;
}
...
async function sendCallDeniedMessage() {
    ...
    const text = substituteParams(
        'I declined {{character}}\'s call because I cannot take a call right now. {{character}}, respond naturally and briefly as if you noticed I cannot take the call. For example, say something like "I see you can\'t take a call right now" or "call me back when you get the chance."',
        { dynamicMacros: { character: callerName } },
    );
    try {
        await sendMessageAsUser(text, undefined, null, true);
...
async function sendOutgoingCallPromptMessage() {
    const callerName = getCharacterNameFallback() || 'the current character';
    const text = substituteParams(
        'I am calling {{character}} now. {{character}}, answer the phone naturally and in character. Use the recent conversation as context, keep it brief, and make it feel like the start of a live phone call.',
        { dynamicMacros: { character: callerName } },
    );
    try {
        await sendMessageAsUser(text, undefined, null, true);
...
async function sendIncomingCallAcceptedMessage() {
    const callerName = getCharacterNameFallback() || 'the current character';
    const text = substituteParams(
        'I answered {{character}}\'s call. {{character}}, greet me naturally and in character as if your call was just picked up. Use the recent conversation as context, keep it brief, and make it feel like the start of a live phone call.',
        { dynamicMacros: { character: callerName } },
    );
    try {
        await sendMessageAsUser(text, undefined, null, true);
...
async function sendIncomingCallMissedMessage() {
    const callerName = getCharacterNameFallback() || 'the current character';
    const text = substituteParams(
        'I missed {{character}}\'s call. {{character}}, respond naturally and briefly as if your call went unanswered. For example, say something like "I tried calling but I guess you missed me" or "call me back when you get the chance."',
        { dynamicMacros: { character: callerName } },
    );
    try {
        await sendMessageAsUser(text, undefined, null, true);
...
function updateUI() {
    ...
    if (micButton.length) {
        if (callMuted) {
            ...
            micButton.prop('title', 'Muted - Click to unmute');
        } else {
            micButton.removeClass('fa-microphone-slash muted').addClass('fa-microphone');
            micButton.prop('title', 'Click to mute');
        }
    }
...
function showCallSubtitleLines(lines, payload = {}) {
    ...
    cleanLines.forEach((line, index) => {
        const showLine = () => {
            if (!callActive || callState !== 'speaking') return;
...
function flushCallUtteranceIfNeeded() {
    if (!callUtteranceHasAudio) {
        return;
    }
    if (sendLiveWsJson({ type: 'flush' })) {
    ...
    try {
        ...
        micStream = await navigator.mediaDevices.getUserMedia({ 
            audio: {
                ...
                noiseSuppression: true,   // Re-enabled: prevents background noise from keeping VAD active
                autoGainControl: true,    // Re-enabled: normalizes audio levels for reliable detection
            }
        });
        ...
        scriptProcessor.onaudioprocess = (e) => {
            ...
            // Downsample to 16kHz before sending (reduces data by ~3x at 48kHz)
            const targetSampleRate = 16000;
            const downsampled = downsampleAudio(inputData, audioContext.sampleRate, targetSampleRate);
            
            // Client-side endpointing: send only speech chunks and flush on silence.
            let sum = 0;
            ...
            const now = Date.now();
            const isSpeech = rms >= getClientNoiseGateThreshold();
            ...
            if (isSpeech) {
                ...
                const shouldKeepShortTail = silenceMs <= CLIENT_SPEECH_HANGOVER_MS;
            ...
            // Efficient base64 encoding using built-in methods
            const uint8View = new Uint8Array(downsampled.buffer);
            const binary = Array.from(uint8View, byte => String.fromCharCode(byte)).join('');
...
export function initCallMode() {
    ...
    $('#voiceforge_call_hypno_ambient_volume_particle').on('input', function() {
        ...
        if (callActive && isHypnoFeatureEnabled('hypnoAmbientEnabled') && callParticleAmbientPlayer) {
            callParticleAmbientPlayer.volume = toParticleAmbientVolume(getHypnoAmbientParticleVolume());
        }
    });
    ...
    $('#voiceforge_call_noise_gate').on('input', function() {
        const pct = Math.max(1, Math.min(100, Number($(this).val()) || 1));
        const rms = uiPercentToRms(pct);
        extension_settings[MODULE_NAME].client_noise_gate = rms;
        $('#voiceforge_call_noise_gate_label').text(`${Math.round(pct)}%`);
        updateMicLevelUi(latestMicRms, micLevelTestActive ? 'test' : (callActive ? 'call' : 'idle'));
    });
    $('#voiceforge_call_noise_gate').on('change', function() {
        const pct = Math.max(1, Math.min(100, Number($(this).val()) || 1));
        extension_settings[MODULE_NAME].client_noise_gate = uiPercentToRms(pct);
        saveSettingsDebounced();
    });

    $('#voiceforge_call_mic_test_button').on('click', async function() {
        if (callActive) {
            updateMicLevelUi(latestMicRms, 'call');
            return;
        }
        if (micLevelTestActive) {
            stopMicLevelTest();
