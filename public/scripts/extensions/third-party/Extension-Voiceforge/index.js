...
function emitVoiceforgeTtsSpokenTextOnce(audioJob, delayMs = 0, durationMs = null) {
    ...
    const dedupeKey = `${audioJob?.messageId ?? 'no_msg'}|${audioJob?.requestId ?? 'no_req'}|${dedupeBasis}`;
...
async function playAudioData(audioJob) {
    ...
    if (audioBlob instanceof Blob && (streamingTtsActive || webAudioIsPlaying) && shouldUseWebAudioStreaming()) {
        try {
            ...
            // Trigger VRM lip sync with Web Audio mode enabled
            if (typeof window['vrmLipSync'] === 'function') {
                window['vrmLipSync'](audioBlob, char, {
                    webAudio: true,
                    startTime: scheduledStartTime + (getEstimatedAudioOutputLatencyMs(webAudioContext) / 1000),
...
function processTtsQueue() {
    ...
    const resolvedBackend = (rawResolvedBackend === 'omnivoice_onnx' || rawResolvedBackend === 'omnivoice_onnx_gpu')
        ? 'omnivoice'
        : rawResolvedBackend;
    if (!resolvedBackend) {
...
function initStreamingTts(messageId, charName) {
    ...
    if (streamingTtsActive && streamingTtsMessageId === messageId) {
    ...
    streamingTtsMessageId = messageId;
...
class VoiceMapEntry {
    ...
    addUI(audioPrompts, rvcModels, bgTracks = []) {
        ...
        const charBackend = (rawCharBackend === 'omnivoice_onnx' || rawCharBackend === 'omnivoice_onnx_gpu')
            ? 'omnivoice'
            : rawCharBackend;
        if (s.tts_backend === 'omnivoice_onnx' || s.tts_backend === 'omnivoice_onnx_gpu') {
            s.tts_backend = 'omnivoice';
        }
        ...
        const template = `
            <div class="voiceforge-voice-entry" id="voiceforge_entry_${sanitizedName}">
                ...
                <div class="voice-content" id="voiceforge_content_${sanitizedName}" style="display: none;">
                    ...
                    <!-- ASMR Section (simplified to match VoiceForge UI) -->
                    <div class="voice-section-collapsible">
                        ...
                        <div class="section-content" id="section_asmr_${sanitizedName}" style="display: none;">
                            <label class="checkbox_label">
                                ...
                                <span>Enable ASMR</span>
                            </label>
                            <div class="post-sliders">
                                <div class="post-slider-row">
                                    <label>Tingles:</label>
                                    <input type="range" class="post-setting" data-key="asmr_tingles" min="0" max="100" value="${post.asmr_tingles}">
                                    <span class="slider-value">${post.asmr_tingles}</span>
                                </div>
                                <div class="post-slider-row">
                                    <label>Breathiness:</label>
                                    <input type="range" class="post-setting" data-key="asmr_breathiness" min="0" max="100" value="${post.asmr_breathiness}">
                                    <span class="slider-value">${post.asmr_breathiness}</span>
                                </div>
                                <div class="post-slider-row">
                                    <label>Crispness:</label>
                                    <input type="range" class="post-setting" data-key="asmr_crispness" min="0" max="100" value="${post.asmr_crispness}">
                                    <span class="slider-value">${post.asmr_crispness}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="post-sliders">
                                <div class="post-slider-row">
                                    ...
                                    <select class="post-setting text_pole" data-key="audio_8d_mode" style="width: 160px;">
                                        <option value="center" ${post.audio_8d_mode === 'center' ? 'selected' : ''}>🎯 Center (Binaural)</option>
                                        <option value="extreme" ${post.audio_8d_mode === 'extreme' ? 'selected' : ''}>👂 Ear Zones (dwell L/R)</option>
                                        <option value="sweep" ${post.audio_8d_mode === 'sweep' ? 'selected' : ''}>↔ Linear Sweep</option>
                                        <option value="rotate" ${post.audio_8d_mode === 'rotate' ? 'selected' : ''}>〰 Smooth Rotation</option>
                                        <option value="static" ${post.audio_8d_mode === 'static' ? 'selected' : ''}>◀ Static Left</option>
                                        <option value="static_right" ${post.audio_8d_mode === 'static_right' ? 'selected' : ''}>▶ Static Right</option>
                                    </select>
                                </div>
                                <div class="post-slider-row">
                                    <label>Quality:</label>
                                    <select class="post-setting text_pole" data-key="audio_8d_quality" style="width: 160px;">
                                        ...
                                        <option value="balanced" ${post.audio_8d_quality === 'balanced' ? 'selected' : ''}>⚖️ Balanced</option>
                                ...
                                <div class="post-slider-row">
                                    <label>Arc (°):</label>
                                    <input type="range" class="post-setting" data-key="audio_8d_depth" min="0" max="360" value="${post.audio_8d_depth || 180}" step="1">
                                    <span class="slider-value">${post.audio_8d_depth || 180}</span>
                                </div>
                                <div class="post-slider-row">
                                    <label>Distance:</label>
                                    <input type="range" class="post-setting" data-key="audio_8d_distance" min="0" max="1" value="${post.audio_8d_distance || 0.3}" step="0.01">
                                    <span class="slider-value">${post.audio_8d_distance || 0.3}</span>
                                </div>
                            </div>
...
jQuery(async function () {
    async function addExtensionControls() {
        const settingsHtml = $(await renderExtensionTemplateAsync('third-party/Extension-Voiceforge', 'settings'));
        const topbarDrawer = $(
